...
public:
	bool OnUserCreate() override
	{
		...
		// Load object file, uncomment for custom obj to be loaded
		//meshCube.LoadFromObjectFile("InsertObjFileNameHere.obj");

		//Hardcoded cube for debugging and demonstration purposes, comment out if custom obj is being loaded
		meshCube.tris = {
		...
		//Sprite loader for use on cube obj for demonstration purposes
		sprTex1 = new olcSprite(L"frog.spr");
						  ...
						  olcSprite* tex)
	{
		...
		if (dy1)
		{
			for (int i = y1; i <= y2; i++)
			{
				int ax{ static_cast<int>(x1 + (float)(i - y1) * dax_step) };
				int bx{ static_cast<int>(x1 + (float)(i - y1) * dbx_step) };
		...
		if (dy1)
		{
			for (int i{ y2 }; i <= y3; i++)
			{
				int ax{ static_cast<int>(x2 + (float)(i - y2) * dax_step) };
				int bx{ static_cast<int>(x1 + (float)(i - y1) * dbx_step) };
